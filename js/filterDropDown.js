(function($){
	...
	$(document).on( 'preInit.dt', function ( e, settings ) 
	{
		...
		$(container).prepend(
			'<div id="' + filterWrapperId + '" class="row filter align-self-center ' + divCssClass + '">' 
			+ filterDef.label + '</div>'
			);
		...
		api.columns(filterDef.columnsIdxList).every( function () 
		{
			...
			$('#' + filterWrapperId)
			.append('<div class="name"><strong>'+colName+':</strong></div><div class="col-md-1" style="max-width: none;"><div class="dropdown"><button class="btn btn-primary dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">'+colName+'</button><dropdown class="dropdown-menu" id="' + selectId 
				+ '" class="' + selectClass + '"></dropdown></div></div>');
			// initalising select for current column and appling event to react to changes
			var select = $("#" + selectId)
				.empty()
				.append( '<option class="dropdown-item" value="">' + 'All' + '</option>' );
	...
	$(document).on( 'init.dt', function ( e, settings ) 
	{
		...
		api.columns(filterDef.columnsIdxList).every( function () 
		{
			...
			// adding select element for current column to container
			//var selectId = id + "_filterSelect" + idx +".dropdown-menu"+" option";									
			// initalising select for current column and appling event to react to changes
			var selectId = id + "_filterSelect" + idx
			var select = $('#'+selectId);
			...
			{
				if (d != "") select.append('<option class="dropdown-item" value="'+d+'">' + d + '</option>' );
			} );
			...
			select.on( 'click', function () 
			{
				...
				var val = $(this).val();
				$(this).parents('.dropdown').find('.dropdown-toggle').html(textval);
				column
